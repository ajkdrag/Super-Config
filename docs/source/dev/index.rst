...
The Super-Config project implements a regression
...
not broken existing functionality. The Super-Config
...
.. code-block:: console
    ...
    (superconfig) $ make test

To run tests verbosely use:

.. code-block:: console

    (superconfig) $ make test-verbose
...
``superconfig`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``superconfig`` package can be found.

.. code-block:: console

    (superconfig) $ cd tests
    (superconfig) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (superconfig) $ python -m test_version
...
.. code-block:: console
    ...
    (superconfig) $ make check-static-analysis
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/superconfig/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (superconfig) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (superconfig) $ make dist-upload

  The package should now be available at https://pypi.org/project/superconfig/
